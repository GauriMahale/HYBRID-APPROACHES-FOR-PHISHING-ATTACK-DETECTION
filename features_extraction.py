...
import urllib # Fetch url
import bs4 #scrap info from web page
import re #string match with re
import socket #two network connection
import whois #register owner of the domain name
...
def favicon(wiki, soup, domain): #loaded from external domain -phsihing
...
def https_token(url): #domain https
...
# Links in <Script> and <Link> tags retrive other web resources
...
def web_traffic(url):
    try:
        rank = \
            bs4.BeautifulSoup(urllib.request.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + url).read(),
                              "xml").find(
                "REACH")['RANK']
    except TypeError:
        return -1
...
def main(url):
    ...
    keys = ["1. Having IP address", "2. URL Length", "3. URL Shortening service", "4. Having @ symbol",
            "5. Having double slash", "6. Having dash symbol(Prefix Suffix)", "7. Having multiple subdomains ",
            " 8. Domain Registration Length", "9. Favicon", "10. HTTP or HTTPS token in domain name", "11. Request URL",
            "12. URL of Anchor",
            "13. Links in tags", "14. SFH", "15. Submitting to email", "16. Abnormal URL",
            "17. IFrame", "18. Age of Domain", "19. DNS Record", "20. Web Traffic", "21. Google Index",
            "22.Statistical Reports"]

    key_value_pairs = dict(zip(keys, status))
    for key in key_value_pairs:
        print(key,":", key_value_pairs[key], end="\n")
    return status
